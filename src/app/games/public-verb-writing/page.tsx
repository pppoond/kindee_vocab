...
import { ArrowLeft, Trophy, Heart, Timer, CheckCircle2, XCircle, RefreshCw, Zap, X } from "lucide-react"
...
export default function PublicVerbWritingPage() {
  ...
  const startGame = (untimedMode?: boolean) => {
    if (verbs.length === 0) {
      showAlert("No irregular verbs found in the public database!", { type: "info" })
      return
    }
    const nextUntimed = untimedMode !== undefined ? untimedMode : isUntimed
    setIsUntimed(nextUntimed)
    setScore(0)
    setLives(3)
    setCurrentIndex(0)
    setTimeLeft(15)
    ...
    return (
      <div className="min-h-screen bg-background text-foreground flex flex-col items-center justify-center p-4">
        ...
        <div className="max-w-md w-full text-center space-y-8">
          <div className="bg-emerald-500/10 p-6 rounded-3xl border border-emerald-500/20 inline-block mb-4">
            <Trophy className="h-16 w-16 text-emerald-400 mx-auto" />
          </div>
          <h1 className="text-4xl font-black bg-gradient-to-r from-emerald-400 to-green-200 bg-clip-text text-transparent">
            Public Verb Writing
          </h1>
          <p className="text-muted-foreground text-sm ThaiFont">
            คุณมีเวลา 15 วินาทีต่อคำ และมี 3 ชีวิต (โหมดสำหรับทุกคน ไม่ต้องเข้าสู่ระบบ)
          </p>
          ...
          <Card className="bg-card border-border p-6 text-left">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between text-sm">
                <span className="text-zinc-500">คำศัพท์กริยาในคลังสาธารณะ</span>
                <span className="font-mono text-emerald-400">{verbs.length} คำ</span>
              </div>
              <div className="h-1.5 w-full bg-muted rounded-full overflow-hidden">
                <div className="h-full bg-emerald-500 transition-all" style={{ width: `${Math.min(100, (verbs.length / 10) * 100)}%` }} />
              </div>
            </div>
          </Card>

          <div className="flex flex-col gap-3">
            <Button size="lg" className="h-14 text-lg font-bold bg-emerald-500 hover:bg-emerald-600 text-white" onClick={() => startGame(false)}>
              Start Game (Timed)
            </Button>
            <Button size="lg" variant="outline" className="h-14 text-lg font-bold border-emerald-500/50 text-emerald-500 hover:bg-emerald-500/10" onClick={() => startGame(true)}>
              Start Game (No Time Limit)
            </Button>
      ...
      <div className="min-h-screen bg-background text-foreground flex flex-col items-center justify-center p-4 text-center">
        ...
        <div className="space-y-8 max-w-sm w-full">
          <div className="relative">
            <Trophy className="h-24 w-24 text-emerald-400 mx-auto animate-bounce" />
          ...
          <div className="flex flex-col gap-3">
            <Button size="lg" className="h-14 text-lg font-bold bg-emerald-500 hover:bg-emerald-600 text-white" onClick={() => startGame()}>
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col p-4">
      <div className="absolute top-4 right-4 flex items-center gap-2">
        <Button variant="ghost" size="icon" asChild className="text-zinc-400 hover:text-foreground">
          <Link href="/games">
            <X className="h-5 w-5" />
          </Link>
        </Button>
      ...
      <div className="max-w-2xl mx-auto w-full flex items-center justify-between py-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className={`flex items-center gap-2 px-4 py-2 rounded-2xl border-2 transition-colors ${!isUntimed && timeLeft < 5 ? "border-red-500/50 bg-red-500/10 text-red-400" : "border-border bg-card"}`}>
            <Timer className={`h-5 w-5 ${!isUntimed && timeLeft < 5 ? "animate-spin" : ""}`} />
            <span className="text-xl font-black tabular-nums">{isUntimed ? "∞" : `${timeLeft}s`}</span>
          </div>
        </div>
      </div>
        ...
        <Card className={`w-full bg-card border-2 transition-all duration-300 ${feedback === "correct" ? "border-emerald-500 shadow-[0_0_30px_rgba(16,185,129,0.2)]" : feedback === "wrong" ? "border-red-500 animate-shake shadow-[0_0_30px_rgba(239,68,68,0.2)]" : "border-border"}`}>
          ...
          <CardContent className="pt-6 pb-12 px-10">
            <form onSubmit={handleSubmit} className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-3">
                  ...
                  <Input 
                    ...
                    className="h-16 text-2xl font-bold bg-zinc-800 border-none focus-visible:ring-emerald-500 text-center"
                    placeholder="..."
                    autoFocus
                    disabled={!!feedback}
                  />
              ...
              <Button type="submit" size="lg" className="w-full h-16 text-xl font-black bg-emerald-500 hover:bg-emerald-600 text-white shadow-xl" disabled={!!feedback}>
